...
/* Start SiteLock SQLi patch */
foreach (array('id') as $vuln) {
  isset($_REQUEST[$vuln]) and $_REQUEST[$vuln] = (int)$_REQUEST[$vuln];
   isset($_GET[$vuln])     and $_GET[$vuln]   = (int)$_GET[$vuln];
   isset($_POST[$vuln])    and $_POST[$vuln]    = (int)$_POST[$vuln];
   isset($$vuln)   and $$vuln  = (int)$$vuln;
  }
/* End SiteLock SQLi patch*/
?>
  ...
  <nav class="topnav navbar navbar-inverse" style="border-color: #E86100;">
        <div class="container-fluid">
          ...
          <?php
           ...
           ?>
          <li><a class="active" href="articles/articles.php"><i class="far fa-newspaper"></i> Articles</a></li>
      ...
      $query= 'SELECT * from post as p INNER JOIN post_text as pt on p.id=pt.id where pt.id='.$post_id;
      $result=$conn->query($query);
      $row = $result->fetch_assoc();
      /*foreach ($row as $col => $val) {
          echo $col." = ".$val."<br>";
      }*/
      echo '<h1 class="articles">'.$row["title"].'</h1>';
    ...
    <div class="comments">
      ...
      <input type="submit" name="" value="comment" class="comments" id=comments></input>';} ?>
